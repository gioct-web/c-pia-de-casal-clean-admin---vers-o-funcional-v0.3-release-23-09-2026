...
}, table => [uniqueIndex("users_open_id_unique").on(table.openId), uniqueIndex("users_username_unique").on(table.username)]);
...
export const pricingRules = mysqlTable("pricing_rules", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [uniqueIndex("pricing_rules_variant_unique").on(table.productKey, table.places, table.itemType, table.fabric), index("pricing_rules_product_idx").on(table.productKey)]);
...
export const estimates = mysqlTable("estimates", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [index("estimates_customer_idx").on(table.customerName), index("estimates_identifier_idx").on(table.id), index("estimates_schedule_idx").on(table.scheduledAt), index("estimates_creator_idx").on(table.createdByUserId)]);
...
export const appSettings = mysqlTable("app_settings", {
  id: int("id").autoincrement().primaryKey(),
  settingKey: varchar("settingKey", { length: 80 }).notNull(),
